...
1. **Wrong environment.** `getEventEnv()` silently returns `"prod"` for anything that isn't exactly `"prod"` or `"staging"` (typos like `"production"`, `"PROD"`, trailing whitespace, or an unset var). Also remember preview mode: a registration submitted from a `?preview=true` session goes to staging. Both environments live in the same dashboard: the event's registrations page filters by data type — `https://app.happily.events/{eventId}/registrations?dataType=prod` (the default view) vs `?dataType=staging` — there is no separate staging deployment to log into. First check: flip that filter to the other environment. If the missing registrations are there, you've found it. Verify `HAPPILY_EVENT_ENV` *in the deployed runtime*, not in your local `.env.local`.
